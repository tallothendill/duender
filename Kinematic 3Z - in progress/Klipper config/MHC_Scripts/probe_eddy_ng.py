...
class ProbeEddy:
    ...
    def define_commands(self, gcode):
        ...
        gcode.register_command(
            ...
            self.cmd_TOOL_TAP,
            "Run the normal EDDYng tap, but send the computed Z to SET_TOOL_Z_OFFSET instead of applying it",
        )
    ...
    def cmd_TOOL_TAP(self, gcmd: GCodeCommand):
        # Same as TAP, but we don't apply any low-level Z changes.
        # We only compute and report the result to a macro.
        drive_current = self._sensor.get_drive_current()
        self._tool_tap_mode = True
        try:
