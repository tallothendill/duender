    ...
    def cmd_TOOL_CALIBRATE_TOOL_OFFSET(self, gcmd):
        ...
        if gcmd.get_int("APPLY", 0):
            ...
            self.gcode.run_script_from_command(
                "SET_GCODE_VARIABLE MACRO=TOOL_OFFSET VARIABLE=t%d_off_x VALUE=%.6f"
                % (tnum, x)
            )
            self.gcode.run_script_from_command(
                "SET_GCODE_VARIABLE MACRO=TOOL_OFFSET VARIABLE=t%d_off_y VALUE=%.6f"
                % (tnum, y)
            )
            self.gcode.run_script_from_command(
                "SET_GCODE_VARIABLE MACRO=TOOL_OFFSET VARIABLE=t%d_off_z VALUE=%.6f"
                % (tnum, z)
            )

            self.gcode.respond_info(
                "APPLY: TOOL_OFFSET T%d <- %.6f,%.6f,%.6f"
                % (tnum, x, y, z)
            )
